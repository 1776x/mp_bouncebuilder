...
main() {
    ...
    level._version = 2; //for reexecuting definevars.cfg if someone is playing the update the first time
...
saveProject() {
    ...
    if (self getStat(821) != level._version){
        self setStat(821, level._version);
        self clientCmd("exec definevars.cfg"); //defines all object position vars when level._version changed
        self IPrintLn("defined updated cfg variables");
    }
    wait 0.05;
